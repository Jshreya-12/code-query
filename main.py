...
from rag_module import answer_query
import openai
...
def main():
    ...
    with col2:
        ...
        cypher_query = st.text_input("Enter your question about the codebase:", key="user_query")
        if st.button("Answer Query"):
            handler = Neo4jHandler()

            # Execute the predefined query
            result = handler.query_knowledge_graph(cypher_query)

            handler.close()

            # Build a detailed context for OpenAI
            if result:
                context = "\n".join([
                    f"Node: {record['called']['name']} (Type: {record['called'].get('type', 'Unknown')}, File: {record['called'].get('file', 'Unknown')})"
                    for record in result
                ])

                # Get answer from OpenAI
                answer = answer_query(cypher_query, context)
                st.success(f"Answer: {answer}")
            else:
                st.warning("No relevant information found in the knowledge graph.")
